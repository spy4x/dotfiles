...
async function main() {
    ...
    for (const app of apps) {
        ...
        if (app.architectures && app.architectures.length > 0) {
            const currentArch = Deno.build.arch === "x86_64" ? "x86_64" : 
                               Deno.build.arch === "aarch64" ? "aarch64" : 
                               Deno.build.arch;
...
gpgkey=https://packages.microsoft.com/keys/microsoft.asc`;
                    ...
                    if (app.repoGpgKey) {
                        ...
                        await runAppCommand(["wget", "-qO-", app.repoGpgKey, "|", "sudo", "apt-key", "add", "-"]);
                    }
            ...
            if (app.flatpak && Deno.build.os === "linux") {
                ...
                const checkInstalled = await runCommand(["flatpak", "list", "--user", `--app=${app.flatpak}`]);
                if (checkInstalled.success && checkInstalled.stdout.includes(app.flatpak)) {
                    ...
                    if (!result.success) {
                        if (nativePackagesFailed) {
                            ...
                            throw new Error(`Both native package and Flatpak installation failed. Native: ${nativePackagesError}. Flatpak: ${result.stderr}`);
                        } else {
    ...
    console.log("---------------------");
    console.log(`✅ Successfully installed: ${successApps.length > 0 ? successApps.join(", ") : "None"}`);
