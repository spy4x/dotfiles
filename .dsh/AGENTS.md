...
`deno task sync-agents-md --apply` copies it byte-for-byte to OpenCode, DSH and
Claude Code (`~/.claude/CLAUDE.md`); `--check` reports drift. Edit the source,
never a copy.
...
Opt-in `skill-state` flow: `~/.config/opencode/skill-state.md`, state in
