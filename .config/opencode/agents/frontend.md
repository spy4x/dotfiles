...
- **Preact + Signals**: prefer Signals over hooks (`useState`, `useEffect`, `useMemo`) when state is reactive. Use hooks only when Signals don't fit (refs, lifecycle, third-party integration).
...
- **Enums**: import from `libs/shared` — same numeric values as backend (enums start at 1 per global AGENTS.md).
