...
## Hard rule: NEVER commit plaintext credentials

No passwords, tokens, API keys, secrets, or private keys in ANY git-tracked file.
Not in `.env.example`, not in scripts, not in docs, not in config, not in comments.
If a file touches git history, assume it's public forever.

Instead:
- `.env` files (gitignored) for local secrets
- SOPS/age-encrypted `.env.age` for committed encrypted secrets
- Scripts read from env vars or source a gitignored `.env` file
...
3. Update gitignored `.env` + re-encrypt `.env.age`
