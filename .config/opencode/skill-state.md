...
This file is the **immutable procedure `P`** you (the agent) follow when
working on a task that runs through `~/.config/opencode/skill-state.md`. It
...
the only safe location — never in `~/.config/opencode/` (that's a symlinked
dotfiles repo; committed secrets/paths would leak).
